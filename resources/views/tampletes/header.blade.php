...
<head>
    ...
    <title>Global Speak - @yield('title')  </title>
</head>

<body>

    <nav class="navbar navbar-expand-lg navbar-dark sticky-top"
        style="background: linear-gradient( #A473FF, #73ABFF );">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ms-auto mb-2 sm-2 mb-lg-0">
                ...
                @auth
                    @if (Auth::user()->is_admin == 1)
                        <a href="{{ route('admin') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Личный кабинет</b></a>

                        <a href="{{ route('logout') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Выйти</b></a>
                    @elseif(Auth::user()->is_admin == 2)
                        <a href="{{ route('superadmin') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Админ панель</b></a>

                        <a href="{{ route('logout') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Выйти</b></a>
                    @else
                        <a href="{{ route('personal') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Личный
                                кабинет</b></a>
                        <a href="{{ route('logout') }}" class="btn btn-outline-light btn-rounded ms-2 btn-lg"><b>Выйти</b></a>
                    @endif
                @endauth
                @guest
                    <button class="btn btn-outline-light btn-rounded ms-2 btn-lg"data-bs-toggle="modal" data-bs-target="#auth" type="submit">
                        <b>Войти</b></button>
                    <button class="btn btn-outline-light ms-2 btn-lg btn-rounded" data-bs-toggle="modal"
                        data-bs-target="#reg"type="submit"><b>Регистрация</b></button>
                @endguest
